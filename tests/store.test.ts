...
// Set env vars BEFORE importing store (which imports config)
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cop-test-"));
process.env.TELEGRAM_BOT_TOKEN = "123456:ABC-DEF";
process.env.TELEGRAM_OWNER_ID = "999";

// Override DATA_DIR by patching the config module
// We need a dynamic import so env vars are set first
